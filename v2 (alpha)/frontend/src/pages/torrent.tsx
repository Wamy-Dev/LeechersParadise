...
const Main = () => {
  ...
  const [apiurl, setApiUrl] = React.useState("http://localhost:3001")//CHANGE THIS TO "http://144.172.80.12:9101" WHEN IN DEV TESTING
  ...
  function deleteDownload(){
    var apiKey = "FBB234HFFJDSFD&&D2C25AD4";
  ...
  const HandleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (entry) {
      ...
      if (currentUser) {
          ...
          .then((obj) => {
            switch (obj.body.status) {
              case 200: //if link is successful
                ...
                setdownload(true);
                break;
              case 403://if not downloaded yet
                setToolTipText("Torrent has not been cached yet. Please try again later.")
                setTooltipOpen(true);
                break;
          ...
          .then((obj) => {
            switch (obj.body.status) {
              ...
              case 403://if not downloaded yet
                ...
                break;
              case 404: //if link is fail
                setToolTipText("Link is invalid.")
                setTooltipOpen(true);
                break;
            }
          }) //if there is some kind of error
        ...
        setToolTipText("Please enter a torrent to download.")
        setTooltipOpen(true);
    }
  };
  //donation value
